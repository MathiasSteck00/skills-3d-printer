...
void line(float newx,float newy,float newz,float newe) {
  ...
  for( i=0; i<maxsteps; ++i ) {
    ...
    //Should it still accelerate? If yes make the delay between steps smaller
    if(i<steps_to_accel) {
      dt -= accel;
    }
    //Should it decelerate? If yes make the delay between steps bigger
    if(i>=steps_to_decel) {
      dt += accel;
    }
  ...
  //Save new position as current position
...
void setup() {
  ...
  resetPosition(); // move to starting position, and set the logical position
  ready();
}
